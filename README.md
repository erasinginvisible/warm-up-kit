...
2. Download the model checkpoint from [link](https://www.dropbox.com/scl/fi/qc5rkqfug0t8oplghxapa/stable_signature.onnx?rlkey=i3cvaqe7emckml4u4o4yqa27d&st=3rorkxl2&dl=0) and put it into the `./models` folder.
3. Download the watermarked images from [link](https://www.dropbox.com/scl/fi/ez4lgdhpve7nhjcrnck31/stable_signature_mscoco.zip?rlkey=6a0nbp6a5rz5ann7apgnaexa0&st=iyasywtu&dl=0) and decompress.

### Attack
1. Develop attacks to remove the watermarks while preserving the quality of the images.
2. Attack the example watermarked images and save them in the same PNG format and filename in a new folder.

### Evaluate
1. Prepare the input (original watermarked images) folder and output (attacked images by participants) folder.
2. Evaluate with `erasinginvisible --path <output_attacked_image_path> --orgpath <input_orginal_image_path>`. A simple report of various watermark performance and image quality metrics will be printed on screen.
